...
inside a filespace: inviting or removing members, permissions, groups,
creating/renaming/deleting filespaces, audit trails, or direct links.
...
- **Always confirm destructive admin actions in the conversation first**, and
  state exactly who/what is affected: inviting or removing a member, revoking
  a permission, deleting or renaming a filespace or group, disabling an audit
  trail.
...
- With several accounts configured, the admin plane binds to the DEFAULT
  account's token - admin tools act on that account's workspace regardless of
  which account the file tools are currently using. Confirm with
  `whoami_admin` before workspace changes when several accounts exist.
...
`remove_member_admin`, `remove_group_member_admin`, `revoke_permission_admin`,
...
Member role changes and service-account lifecycle (mint/rotate/delete) are not
exposed through the MCP - direct the user to the LucidLink web app for those.
